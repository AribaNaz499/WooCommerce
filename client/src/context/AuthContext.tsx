...
const invalidateCatalogQueries = () => {
  ...
  const safariCatalogRefresh =
    typeof window !== "undefined" && isSafariBrowser();

  if (!safariCatalogRefresh) {
    // Non-Safari: force a fully fresh network read.
    clearCatalogCaches();
  }
  ...
  if (safariCatalogRefresh) {
    // Safari: keep visible catalog data on screen and refresh in background.
